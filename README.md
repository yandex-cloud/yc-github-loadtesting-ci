...
|`agent-filter`|<p>A filter expression to select agents to execute tests.</p><p>Example:<br />  - 'name contains github' - agents with 'github' substring in name.<br />  - 'labels.workflow=$WORKFLOW_ID' - agents with label 'workflow' equals to $WORKFLOW_ID</p>|`${{ github.run_id }}`|no|
